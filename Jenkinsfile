pipeline {
    ...
    stages {
	 ...
	 stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user tienvc:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://54.215.81.76:8080/job/lab_cicd/buildWithParameters?token=gitops-token'"
                }
            }
         }
    }
